...
sunapee |>
  ...
  labs(
    x = "Year",
    y = "Ice off day",
    title = "Ice off days through the years"
  ) +
  theme_classic()
```

# How does ice cover change over time at Lake Sunapee?
Ice-off dates at Lake Sunapee occur earlier over time as shown by lower day of year values in recent years. Even though there is year to year variability, the overall trend is suggesting an earlier spring melt. 
...
**Answer 2:**
I expect the slope to be negative due to the fact that ice off is occurring earlier in the year over time. So as year increases, ice_off_doy decreases, creating a negative slope. 
...
**Answer 3:**
The slope is negative as expected from the previous question. 
# Is our model any good?
I think the model does a decent job of predicting ice off dates, assuming that ice off dates are linear. 
## R2
...
**Answer 4:**
The R^2 value is 0.2055 since its closer to 0 year and ice_off_doy are not strongly related.  
...
**Answer 6:**
The residuals are roughly centered around 0 and symmetric. While the distribution is not perfectly normal, it does not have a strong skew to either side or any extreme outliers. We can reasonably assume that the distribution meets the assumptions of a linear model. 
```{r}
sunapee |>
  ggplot(aes(resid)) +
...
So based off the slope it would take approximately 66 years for ice off to occur one week earlier. This is meaningful because a one week shift would impact lake ecology greatly. 
...
**Answer 9:**
I am moderately comfortable using this model to describe long term trends but less comfortable using it for precise prediction. Even though the slope is statistically significant and the residual distribution is roughly normal which does suggest linear modeling assumptions were met, the R^2 value indicated only about 20% of the variation in ice off is explained by year. Meaning that most the variability is due to other factors. Therefore, the model only explains overall trends but does not explain year to year fluctutations. 
...
sunapee |>
  ...
  labs(
    x = "Year",
    y = "Ice off DOY",
    title = "Ice off DOY with Linear Regression line"
  ) +
  theme_classic()
...
**Answer 11:**
The predicted ice off day in 2040 is approximately day 103 of the year. Using the 5% and 95% residuals to construct a 90% CI, the predicted range is approximately 83 to 117 days. This gives us a range of about 34 days.
```{r}
...
prediction <- 318.94856 + (-0.10602 * 2040)

lower_ci <- prediction + lower
upper_ci <- prediction + upper
...
fit_before <- lm(ice_off_doy ~ year, data = before) 

fit_after <- lm(ice_off_doy ~ year, data = after)
...
sunapee |>
  ...
  geom_line() +
  geom_abline(
    intercept = fit_before$coefficients[1],
    slope = fit_before$coefficients[2],
    color = "red",
  ) +
  geom_abline(
    intercept = fit_after$coefficients[1],
    slope = fit_after$coefficients[2],
    color = "blue"
  ) +
  labs(
    x = "Year",
    y = "Ice off DOY",
    title = "Ice off trends before and after 1970"
  ) +
  theme_classic()
```

The slope before 1970 is -0.077 and the slope after 1970 is -0.302. Because the slope post 1970 is much more negative, ice off dates are shifting earlier at a faster rate in recent decades than it was. 
